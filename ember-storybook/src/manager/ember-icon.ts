...
/**
 * The Ember "e-circle" brand icon, embedding the markup from
 * https://github.com/ember-learn/ember-website/blob/main/public/images/brand/Ember%20Logos/Icon/e-circle-icon-4c.svg
 *
 * Storybook 10 resolves toolbar `icon` strings against a fixed manager-side
 * map, so a custom brand icon has to be a React element rendered by our own
 * toolbar tool. The fills are hardcoded brand colors on purpose: unlike
 * Storybook's monochrome icons this one must keep its red/white identity.
 */
const EMBER_ICON_MARKUP = `
...
export function EmberIcon(props: SVGProps<SVGSVGElement>) {
  return createElement('svg', {
    ...
    'aria-hidden': true,
    ...props,
    // The markup mirrors the upstream brand asset; static, first-party content.
    dangerouslySetInnerHTML: { __html: EMBER_ICON_MARKUP }
  });
}
